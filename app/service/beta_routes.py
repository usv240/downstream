...
from downstream.store import ScopedWorkspaceStore, WorkspaceStore
...
def build_beta_router(store: WorkspaceStore, auth: ApiKeyAuthenticator) -> APIRouter:
    router = APIRouter(prefix="/v1", tags=["beta-api"])

    def scoped(principal: ApiPrincipal) -> ScopedWorkspaceStore:
        require_scope(principal, "downstream:use")
        return ScopedWorkspaceStore(store, principal.tenant_id)

    def require(
        workspace_id: str, principal: ApiPrincipal
    ) -> tuple[ScopedWorkspaceStore, dict[str, Any]]:
        owned = scoped(principal)
    ...
    def open_workspace(
        ...
        workspace = create_workspace(
            dam=_dam_from_nid(result.records[0], result.source_url), facts=[]
        )
        owned = scoped(principal)
        owned.put(workspace)
    ...
    def get_workspace(
        workspace_id: str, principal: ApiPrincipal = Depends(auth)
    ) -> dict[str, Any]:
        return public_view(require(workspace_id, principal)[1])
    ...
    def answer(
        ...
        owned, workspace = require(workspace_id, principal)
        try:
            record_answer(
                workspace,
                request.question_id,
