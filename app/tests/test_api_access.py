...
def test_key_resolves_tenant_and_openapi_security_scheme():
    auth = ApiKeyAuthenticator.from_plaintext({"ds_test_secret": {"tenant_id": "owner_one", "label": "Owner one", "scopes": ["downstream:use"]}})
    api = app_for(auth)
    assert api.get("/v1/whoami", headers={"X-API-Key": "ds_test_secret"}).json()["tenant_id"] == "owner_one"
